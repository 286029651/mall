...
public class Swagger2Config {
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
                ...
                .apis(RequestHandlerSelectors.basePackage("com.ralph.mall.controller")).paths(PathSelectors.any())
                .build()
                // 添加登录认证
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("SwaggerUI演示").description("mall-tiny").contact("ralph").version("1.0")
                .build();
    }
